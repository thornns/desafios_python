...
Algumas empresas definem que um cliente é considerado um cliente inadimplente quando ele está devendo acima de
X reais e por mais de X dias.

Algumas empresas definem que um vendedor bateu a meta quando ele vendeu acima de X reais
(outras ainda analisam não só as vendas desse vendedor, mas também da loja ou da unidade de negócios que ele faz parte,
ou ainda uma avaliação qualitativa)

Algumas empresas definem que um produto está em "falta" no estoque quando ele está abaixo de um nível mínimo
quando um cliente insere um novo pedido daquele produto

E assim vai para dezenas de indicadores.

O ponto importante é, cada empresa tem alguma adaptação do cálculo de indicadores.
E normalmente se formos analisar esses indicadores é interessante ter funções que façam
o trabalho de análise deles. Por isso, vamos construir 2 exemplos aqui:
...
def stock_out(venda):
    ...
    y = 0
    for chave in venda:
        if 'Estoque' in venda[chave][2]:
            x += venda[chave][0]
        elif 'Concluído' in venda[chave][1]:
            y += venda[chave][0]
    retorno = x / (y + x)
    return retorno


stockout = stock_out(vendas)
print("Stockout de {:.2%}".format(stockout))
...
cobrança poder fazer a cobrança dos clientes.

Sua função deve então receber uma lista de clientes, analisar quais clientes estão inadimplentes,
e retornar uma lista com os clientes inadimplentes (apenas o CPF deles já é suficiente)
...
Isso significa que caso ou cliente esteja devendo 2.000 reais a 10 dias, ele não é inadimplente,
pois não se passaram 20 dias ainda. Da mesma forma, se ele estiver devendo 500 reais por 40 dias,
...
def listar_devedores(cliente_devedor):
    lista_inadimplentes = []
    for i in range(len(cliente_devedor)):
        if cliente_devedor[i][1] > 1000 and cliente_devedor[i][2] > 20:
            lista_inadimplentes.append(cliente_devedor[i][0])
    return lista_inadimplentes


print(listar_devedores(clientes_devedores))
